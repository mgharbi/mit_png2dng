...
void dng_render_task::Start (uint32 threadCount,
	...
	if (!fNegative.IsMonochrome ())
        ...
        // MIKA
		// fCameraToRGB = dng_space_sRGB::Get ().MatrixFromPCS () *
		// 			   spec->CameraToPCS ();
    ...
    printf("Render:exposure %f\n", exposure);
        ...
        printf("Render:black %f\n", black);
					   
		black = Min_real64 (black, 0.99 * white);

        printf("Render:black_corrected %f\n", black);
        ...
        printf("Render:exposure_ramp slope %f\n", rampFunction.fSlope);
        printf("Render:exposure_ramp black %f\n", rampFunction.fBlack);
        printf("Render:exposure_ramp radius %f\n", rampFunction.fRadius);
        printf("Render:exposure_ramp qscale %f\n", rampFunction.fQScale);
        printf("Render:exposure_ramp at .5 %f\n", rampFunction.Evaluate(0.5));
	...
	// Compute tone curve.
        ...
        printf("Render:exposureTone at .5 %f\n", exposureTone.Evaluate(0.5));
        printf("Render:tonecurve at .5 %f\n", fParams.ToneCurve().Evaluate(0.5));
        printf("Render:total_tone at .5 %f\n", totalTone.Evaluate(0.5));
...
dng_render::dng_render (dng_host &host,
	...
	if (profile && profile->ToneCurve ().IsValid ())
        ...
        printf("Render:profile has tone curve! %f\n");
